...
const addMembers = TryCatch(async (req, res, next) => {
    ...
    if (chat.members.length > 100)
        return next(new ErrorHandler("Group chat can't have more than 100 members", 400));
...
const sendAttachments = TryCatch(async (req, res, next) => {
    ...
    if(files.length > 1) return next(new ErrorHandler("Files can't be more than 5", 400));
...
const deleteChat = TryCatch(async (req, res, next) => {
    ...
    // Here we have to delete All messages as well as attachments & files from cloudinary
